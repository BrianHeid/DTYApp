...
Meteor.methods({
	...
	pushRequest: function(id, info, requesterName){
		...
		// if (Requests.find({_id:id}).fetch().length > 0){
			...
			// 	"address": info.street + ' ' + info.aptNumSuite + ' ' + info.city + ' ' + info.activeStates + ' ' + info.zipcode,
			// 	"times.laterTime": info.laterTime,
			// 	"times.preferredTime": info.time,
			// 	"times.preferredDate": info.date,
			// 	"times.requestTime": new Date().toLocaleString(),
		...
		// else{
			...
			var times = {laterTime: info.laterTime, preferredTime: info.time, preferredDate: info.date, requestTime: new Date().toLocaleString(),
							callTime: "", callDuration: 0, reservedTime: ""};

			var treatment = {requestId: "", isHouseCall: true, providerName: "", datetime: "", duration: 0, ETA: "", address: ""};
			var followups = [{}];
	...
	updateRequestedDate: function(requestId, newDate){
		return Requests.update({_id: requestId}, {$set: {'times.preferredDate': newDate}});
	},

	updateRequestedTime: function(requestId, newTime){
		return Requests.update({_id: requestId}, {$set: {'times.preferredTime': newTime}});
	},

	acceptRequest: function(id){
		Requests.update({_id: id}, {$set: {'accepted': true}});
	},

	cancelRequest: function(requestId, cancelReason, cancelAfterReserve){
		Requests.update({_id: requestId}, {$set: {'cancelReason': cancelReason, 'cancelAfterReservation': cancelAfterReservation}});
	},
